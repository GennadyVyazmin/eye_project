# video_analytics_trassir_cuda.py
import cv2
import numpy as np
import sqlite3
import datetime
...
try:
    ...
    if CUDA_AVAILABLE:
        ...
        if gpus:
            ...
            logger.info(f"🎯 CUDA доступно: {len(gpus)} GPU")
    else:
        logger.warning("⚠️ CUDA недоступно, используется CPU")
except ImportError:
...
class CUDATrassirCounter:
    def __init__(self, processing_interval=0.5, similarity_threshold=0.55, tracking_threshold=0.45):
        """
        CUDA-ускоренная версия системы аналитики
        """
        self.conn = sqlite3.connect('visitors_trassir_cuda.db', check_same_thread=False)
        self._init_database()
        ...
        self.photos_dir = "visitor_photos_cuda"
        ...
        self.frame_queue = Queue(maxsize=2)  # Увеличили буфер для GPU
        self.results_queue = Queue()

        # CUDA-ускоренный детектор лиц
        self.detector_backend = self._select_best_detector()

        # Инициализация детектора один раз для производительности
        ...
        logger.info(f"🚀 CUDA система инициализирована. Детектор: {self.detector_backend}")

    def _select_best_detector(self):
        """Выбор лучшего доступного детектора с поддержкой GPU"""
        detectors_priority = [
            'retinaface',  # Лучшая точность, поддерживает GPU
            'mtcnn',  # Хорошая точность, поддерживает GPU
            'opencv',  # Быстрый, но только CPU
            'ssd',  # Быстрый детектор
        ]

        for detector in detectors_priority:
            try:
                # Тестируем детектор на маленьком изображении
                test_img = np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)
                DeepFace.extract_faces(test_img, detector_backend=detector, enforce_detection=False)
                logger.info(f"✅ Выбран детектор: {detector}")
                return detector
            except Exception as e:
                logger.warning(f"❌ Детектор {detector} недоступен: {e}")
                continue

        logger.warning("⚠️ Все детекторы недоступны, используем opencv")
        return 'opencv'

    def _warm_up_detector(self):
        """Прогрев детектора для избежания задержек при первом запуске"""
        logger.info("🔥 Прогрев детектора...")
        try:
            warm_up_img = np.random.randint(0, 255, (200, 200, 3), dtype=np.uint8)
            DeepFace.extract_faces(warm_up_img, detector_backend=self.detector_backend, enforce_detection=False)
            logger.info("✅ Детектор прогрет")
        except Exception as e:
            logger.warning(f"⚠️ Ошибка прогрева детектора: {e}")
    ...
    def detect_faces_cuda(self, frame):
        """CUDA-ускоренная детекция лиц"""
        try:
            # Используем DeepFace с выбранным бэкендом
            ...
            for face_obj in faces:
                if 'facial_area' in face_obj:
                    ...
                    # Фильтруем слишком маленькие/большие лица
                    if 30 <= w <= 400 and 30 <= h <= 400:
                        detected_faces.append((x, y, w, h))

            logger.debug(f"🔍 CUDA детекция: найдено {len(detected_faces)} лиц")
            return detected_faces

        except Exception as e:
            logger.error(f"❌ Ошибка CUDA детекции: {e}")
            # Fallback на OpenCV
            return self.detect_faces_fallback(frame)

    def detect_faces_fallback(self, frame):
        """Резервный детектор на OpenCV"""
        ...
        logger.info(f"🔍 Fallback детекция: {len(faces)} лиц")
        return faces

    def get_fast_embedding(self, face_image):
        """Быстрое получение эмбеддинга с GPU ускорением"""
        try:
            # Используем GPU-ускоренное получение эмбеддинга
    ...
    def calculate_similarity(self, embedding1, embedding2):
        """Расчет схожести лиц с оптимизацией для GPU"""
        if embedding1 is None or embedding2 is None:
            return 0.0

        try:
            # Используем оптимизированные numpy операции
    ...
    def find_best_match(self, embedding):
        """Поиск лучшего совпадения с векторными операциями"""
        if embedding is None:
            return None, 0.0

        if not self.known_visitors_cache:
            return None, 0.0

        try:
            # Векторизованный расчет схожести для производительности
            known_embeddings = np.array(list(self.known_visitors_cache.values()))
            similarities = np.dot(known_embeddings, embedding)

            norms_known = np.linalg.norm(known_embeddings, axis=1)
            norm_input = np.linalg.norm(embedding)

            similarities = similarities / (norms_known * norm_input)

            best_idx = np.argmax(similarities)
            best_similarity = similarities[best_idx]

            if best_similarity > 0:  # Минимальный порог
                best_match_id = list(self.known_visitors_cache.keys())[best_idx]
            ...
            return self.find_best_match_fallback(embedding)

    def find_best_match_fallback(self, embedding):
        """Резервный последовательный поиск"""
        best_match_id = None
        best_similarity = 0.0
    ...
    def confirm_visitor_identity(self, track_id, face_data):
        ...
        if similarity > self.similarity_threshold:
            ...
            if track_duration > 1.5:  # Уменьшили время ожидания благодаря CUDA
    ...
    def _processing_worker(self):
        """Фоновая обработка кадров с GPU ускорением"""
        while not self.stop_processing:
            try:
                frame_data = self.frame_queue.get(timeout=1.0)
                frame, frame_time = frame_data

                result = self._process_frame_cuda(frame)
    ...
    def _process_frame_cuda(self, frame):
        """Тяжелые операции обработки с CUDA"""
        try:
            # CUDA-ускоренная детекция лиц
            faces = self.detect_faces_cuda(frame)

            processed_faces = []
            if len(faces) > 0:
                self.recognition_stats['total_detections'] += len(faces)
                logger.info(f"👥 CUDA ОБНАРУЖЕНО ЛИЦ: {len(faces)}")
            ...
            logger.error(f"❌ Ошибка в CUDA обработке: {e}")
    ...
    def process_frame_realtime(self, frame):
        """Обработка кадра в реальном времени с GPU"""
        current_time = time.time()
        ...
        # Более частая обработка благодаря CUDA
    ...
    def start_analysis(self, rtsp_url):
        """Запуск анализа с CUDA"""
        logger.info("🚀 Запуск CUDA-ускоренной версии...")
        ...
        logger.info("✅ CUDA анализ запущен")

        window_name = 'Trassir Analytics - CUDA ACCELERATED'
        ...
        try:
            ...
            logger.info(f"📊 ФИНАЛЬНАЯ СТАТИСТИКА CUDA:")
            ...
            logger.info("✅ CUDA анализ завершен")
...
def main():
    ...
    counter = CUDATrassirCounter(
        processing_interval=0.5,  # Уменьшили интервал благодаря CUDA
